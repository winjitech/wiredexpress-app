...
class _TrackingMapWidgetState extends State<TrackingMapWidget> {
  ...
  void initState() {
    super.initState();
    //Map<String?, dynamic> coverage = Provider.of<SplashProvider>(context, listen: false).configModel!.storeLocationCoverage!;
  ...
  //  _storeLatLng = LatLng(coverage['latitude'], coverage['longitude']);
  }
  ...
  void setMarker() async {
   // Uint8List storeImageData = await convertAssetToUnit8List(Images.store_marker, width: ResponsiveHelper.isMobilePhone() ? 50 :  15);
    Uint8List deliveryBoyImageData = await convertAssetToUnit8List(Images.delivery_boy_marker, width: ResponsiveHelper.isMobilePhone() ? 50 :  15);
    Uint8List destinationImageData = await convertAssetToUnit8List(Images.destination_marker, width: ResponsiveHelper.isMobilePhone() ? 50 :  15);
    ...
    if(_controller != null) {
      ...
      _rotation = 0;
      // if (_addressLatLng!.latitude < _storeLatLng!.latitude) {
      // }else {
      //   bounds = LatLngBounds(southwest: _storeLatLng!, northeast: _addressLatLng!);
      //   _rotation = 180;
      // }
    }
    ...
    // _markers.add(Marker(
    //   markerId: MarkerId('Delivery Man'),
    //   position: _storeLatLng!,
    //   infoWindow: InfoWindow(
    //     title: 'Delivery Man',
    //     snippet: '${_storeLatLng!.latitude}, ${_storeLatLng!.longitude}',
    //   ),
