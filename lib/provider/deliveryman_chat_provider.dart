...
class DeliveryManChatProvider extends ChangeNotifier {
  final DeliveryManChatRepo? chatRepo;
  DeliveryManChatProvider({@required this.chatRepo});
  ...
  void getChatList(BuildContext? context, String orderId) async {
  //  _chatList = null;
    _imageFile = null;
    ApiResponse apiResponse = await chatRepo!.getChatList(orderId);
  ...
  Future<void> sendMessage(String message, String token, String userID,String orderId, BuildContext? context) async {
    http.StreamedResponse response = await chatRepo!.sendMessage(message, token, orderId);
  ...
  Future<ResponseModel> sendImage(BuildContext context,File file, String token, String message, String orderId) async {
    _imageLoading = true;
    notifyListeners();
    ResponseModel _responseModel;
    http.StreamedResponse response = await chatRepo!.sendImage(file, token, message, orderId);
    // _isLoading = false;
  ...
  Future<List<ChatModel>> refresh(BuildContext context,String orderId, bool instantly) async{
    print('chat test -1');
